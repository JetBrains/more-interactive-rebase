...
import java.util.*
//TODO(no clue what is happening here just deal with it later)
class IRHistoryTraverser (private val project: Project, private val logData: VcsLogData) : GitHistoryTraverser {
    ...
    private fun startSearch(
            ...
            commitHandler: GitHistoryTraverser.Traverse.(id: GitHistoryTraverser.TraverseCommitInfo) -> Boolean
    ) {
        ...
        walker(hashNodeId, graph, visited) {
            ...
            traverse.commitHandler(GitHistoryTraverser.TraverseCommitInfo(commitId, parents))
        }
    ...
    private class IndexedRootImpl(
        ...
        override fun loadMetadata(id: TraverseCommitId): VcsCommitMetadata {
            ...
            return IndexedDetails.Companion.createMetadata(id, dataGetter, storage, factory)!!
        }
    }

    private class RequestedRootsIndexingListenerImpl(
            private val requestedRoots: Collection<VirtualFile>,
            private val traverser: IRHistoryTraverser,
    ...
    private class TraverseImpl(private val traverser: GitHistoryTraverser) : GitHistoryTraverser.Traverse {
